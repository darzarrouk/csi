...
# ----------------------------------------------------------------------
# Compute the Green's functions for the patches using fomosto and mpi4py
def sum_layered_fomosto(xs, ys, zs, strike, dip, rake, area, slip, xr, yr, stratKernels):
    ...
    # Some initializations
    Np = len(xs)            # number of sources
    nrec = int(len(xr)) # number of receivers
    ...
    point_sources = np.column_stack([xs, ys, zs, strike, dip, rake, M])

    uxBig = np.zeros((Np, nrec)).astype('float32')
    uyBig = np.zeros((Np, nrec)).astype('float32')
    uzBig = np.zeros((Np, nrec)).astype('float32')

    for i in range(Np):
        counter = i % (size - 1) + 1
        if (rank == 0):
            comm.Isend(point_sources[i, :], dest=counter, tag=i)
        elif (rank == counter):
            recvbuf = np.zeros((1, 7))
            comm.Recv(recvbuf, source=0, tag=i)
            dc_source = gf.DCSource(lat = 0.0, lon = 0.0, east_shift = recvbuf[0, 0], north_shift = recvbuf[0, 1], depth = recvbuf[0, 2], strike = recvbuf[0, 3], dip = recvbuf[0, 4], rake = recvbuf[0, 5], magnitude = recvbuf[0, 6])
            result = engine.process(dc_source, target)
            disp = result.results_list[0][0].result
            ux = disp['displacement.e']
            uy = disp['displacement.n']
            uz = disp['displacement.d'] * -1.0
            uxBig[i, :] = ux
            uyBig[i, :] = uy
            uzBig[i, :] = uz
        else:
    ...
    for i in range(Np):
        counter = i % (size - 1) + 1
        recvbuf = np.zeros((3, nrec)).astype('float32')
        if (rank == counter):
            recvbuf = np.vstack([uxBig[i, :], uyBig[i, :], uzBig[i, :]])
        comm.Bcast(recvbuf, root=counter)
        uxBig[i, :] = np.copy(recvbuf[0, :])
        uyBig[i, :] = np.copy(recvbuf[1, :])
        uzBig[i, :] = np.copy(recvbuf[2, :])

    if ((rank == 0) or (rank == 1)):
        print(uxBig, np.min(uxBig), np.max(uxBig))

    # return the GF matrices
    return [uxBig.T, uyBig.T, uzBig.T]
# ----------------------------------------------------------------------
